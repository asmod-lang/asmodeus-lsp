...
fn test_create_uppercase_action() {
    ...
    let uppercase_action = actions.iter().find(|action| {
        match action {
...
fn test_create_add_comment_action() {
    ...
    let comment_action = actions.iter().find(|action| {
        match action {
...
fn test_create_format_instruction_action() {
    ...
    let format_action = actions.iter().find(|action| {
        match action {
...
fn test_create_add_label_action() {
    ...
    let label_action = actions.iter().find(|action| {
        match action {
            CodeActionOrCommand::CodeAction(action) => action.title == "Add label above",
            _ => false,
        }
    });
