...
fn test_analyzer_hover_delegation() {
    let analyzer = SemanticAnalyzer::new();
    let hover = analyzer.get_hover_info("POB #42", Position { line: 0, character: 1 });
...
fn test_analyzer_definition_delegation() {
    ...
    let definition = analyzer.get_definition(content, Position { line: 1, character: 8 }, &uri);
...
fn test_analyzer_signature_help_delegation() {
    ...
    let signature = analyzer.get_signature_help(content, Position { line: 0, character: 4 });
...
fn test_analyzer_rename_delegation() {
    ...
    let edit = analyzer.rename_symbol(content, Position { line: 0, character: 2 }, "begin", &uri);
    assert!(edit.is_some());
}
