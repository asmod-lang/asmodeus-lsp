...
#[test]
fn test_get_rename_range_for_label() {
    let provider = SymbolProvider::new();
    let content = r#"start:
    POB #42
    SOB start"#;
...
#[test]
fn test_get_rename_range_for_instruction_denied() {
    let provider = SymbolProvider::new();
    let content = "POB #42";
    
    let range = provider.get_rename_range(content, Position { line: 0, character: 1 });
    
    assert!(range.is_none()); // cannot rename instruction
}

#[test]
fn test_get_rename_range_invalid_symbol() {
    let provider = SymbolProvider::new();
    let content = "123invalid #42";
    
    let range = provider.get_rename_range(content, Position { line: 0, character: 1 });
    
    assert!(range.is_none()); // invalid symbol name
}
