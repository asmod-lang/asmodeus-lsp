use crate::analysis::utils::{position_to_range, InstructionDatabase};
...
impl RefactoringActionsProvider {
    ...
    pub fn get_refactoring_actions(
        &self,
        content: &str,
        range: Range,
        uri: &Url,
    ...
    fn create_uppercase_action(
        ...
        if self
            .instruction_db
            .is_valid_instruction(&selected_text.to_uppercase())
    ...
    fn create_add_comment_action(
        ...
        if !line.contains(';') && !line.trim().is_empty() {
            let edit = TextEdit {
                range: position_to_range(range.start.line, line.len() as u32, line.len() as u32),
    ...
    fn create_format_instruction_action(
        &self,
        content: &str,
        range: Range,
        uri: &Url,
        ...
        if !trimmed.is_empty() && !line.starts_with("    ") {
            ...
            let edit = TextEdit {
                range: position_to_range(range.start.line, 0, line.len() as u32),
    ...
    fn create_add_label_action(
        ...
        if !trimmed.is_empty() && !trimmed.contains(':') {
            ...
            if !words.is_empty() && self.instruction_db.is_valid_instruction(words[0]) {
                let edit = TextEdit {
                    range: position_to_range(range.start.line, 0, 0),
                    new_text: "label:\n".to_string(),
                };
                ...
                return Some(CodeActionOrCommand::CodeAction(CodeAction {
                    title: "Add label above instruction".to_string(),
                    kind: Some(CodeActionKind::REFACTOR),
                    diagnostics: None,
                    edit: Some(WorkspaceEdit {
                        changes: Some(changes),
